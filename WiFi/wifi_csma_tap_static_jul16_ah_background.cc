...
main (int argc, char *argv[])
{
  bool verbose = true;
  //uint32_t i,j;
  uint32_t nWifi11 = 5,duration = 500;
  uint32_t nCsma10 = 2;
  uint32_t np2p9 = 2;
  bool tracing = true;
...
for(int i = 1 ; i<=4;i++)
  {wifiDevices11.Add (wifi.Install (wifiPhy, mac, NodeContainer (wifiNodes11.Get (i))));}
...
for(int i = 1 ; i<=4;i++)
  {mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
    ...
    "GridWidth", UintegerValue (3),
    "LayoutType", StringValue ("RowFirst"));
  mobility.Install (wifiNodes11.Get(i));}
  ...
  //Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
   ...
   /*background traffic*/
  UdpServerHelper* udpServer = new UdpServerHelper[18];
  ApplicationContainer* serverApps = new ApplicationContainer[18];
  UdpClientHelper* udpClient = new UdpClientHelper[18];
  ApplicationContainer* clientApps = new ApplicationContainer[18];

  for (int i = 0; i <  1; i++)
    {
      // set server port to 100+(i*10)
      udpServer[i] = UdpServerHelper (100 + (i * 10));
      serverApps[i] = udpServer[i].Install (wifiNodes11.Get (i+2));
      serverApps[i].Start (Seconds (1));
      serverApps[i].Stop (Seconds (duration));

      udpClient[i] = UdpClientHelper (WifiInterfaces11.GetAddress (i+2), 100 + (i * 10));
      ...
      clientApps[i] = udpClient[i].Install (wifiNodes11.Get (i + 3));
  ...
  if (tracing == true)
    {
      ...
      csma10.EnablePcap ("wifiah-csma-tap-jul11", csmaDevices10.Get (1), true);
      wifiPhy.EnablePcap ("wifiah-csma-tap-jul11", wifiDevices11.Get (0), true);
    }
