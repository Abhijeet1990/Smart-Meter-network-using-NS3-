...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("tapName3","Name of the OS thrd tap device", tapName3);
  cmd.Parse (argc,argv);
  ...
  pointToPoint8.SetDeviceAttribute("DataRate",StringValue("1Mbps"));
  pointToPoint8.SetChannelAttribute("Delay",StringValue("8ms"));
  NodeContainer p2pNodes8 =NodeContainer(wifiNodes11.Get(1),csmaNodes12.Get(0));
  NetDeviceContainer p2pDevices8 = pointToPoint8.Install(p2pNodes8);
...
//StaWifiMac: The Wifi MAC high model for a non-AP STA in a BSS. 
  ...
  wifiDevices11.Add (wifi.Install (wifiPhy, wifiMac, NodeContainer (wifiNodes11.Get (1))));
  ...
  tapBridge3.Install (csmaNodes12.Get (1), csmaDevices12.Get (1));
  
Ipv4StaticRoutingHelper ipv4RoutingHelper;
  
  Ptr<Node> node1 = p2pNodes9.Get(0);
  Ptr<Node> node2 = p2pNodes9.Get(1);
  Ptr<Node> node3 = p2pNodes8.Get(1);
  ...
  Ipv4Address node1_if3 = node1_addr->GetAddress(3,0).GetLocal();
  std::cout << "p2p node 1 interface one: "<<node1_if1 << "p2p node 1 interface two:"<<node1_if2<<"p2p node 1 interface three:"<<node1_if3<<std::endl;
  ...
  p2pnode1->AddNetworkRouteTo (Ipv4Address ("12.12.12.0"), Ipv4Mask ("255.255.255.0"), 2);
  ...
  p2pnode2->AddNetworkRouteTo (Ipv4Address ("11.11.11.0"), Ipv4Mask ("255.255.255.0"), 1);
   ...
   Ptr<Ipv4StaticRouting> p2pnode3 = ipv4RoutingHelper.GetStaticRouting (node3->GetObject<Ipv4>());
  p2pnode3->AddNetworkRouteTo (Ipv4Address ("11.11.11.0"), Ipv4Mask ("255.255.255.0"), 1);
  ...
  if (tracing == true)
    {
       pointToPoint8.EnablePcap("wifi-csma-tap-jul25-attack", p2pDevices8.Get (0), true);
      pointToPoint8.EnablePcap("wifi-csma-tap-jul25-attack", p2pDevices8.Get (1), true);
      //pointToPoint.EnablePcap("wifi-csma-tap-jul25-attack", p2pDevices9.Get (0), true);
      //pointToPoint.EnablePcap("wifi-csma-tap-jul25-attack", p2pDevices9.Get (1), true);
      //csma10.EnablePcap ("wifi-csma-tap-jul25-attack", csmaDevices10.Get (1), true);
      //wifiPhy.EnablePcap ("wifi-csma-tap-jul25-attack", wifiDevices11.Get (1), true);
      //wifiPhy.EnablePcap ("wifi-csma-tap-jul25-attack", wifiDevices11.Get (0), true);
    }
