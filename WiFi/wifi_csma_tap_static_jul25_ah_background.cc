...
main (int argc, char *argv[])
{
  bool verbose = true;
  //uint32_t i,j;
  uint32_t nWifi10 = 50,duration = 359;
  uint32_t nCsma11 = 2;
  uint32_t np2p9 = 2;
  bool tracing = true;
...
uint32_t  NRawSlotCount = 829;
  ...
  double packet_mean = 800;
  double interval_mean = 2;
  Ptr<ExponentialRandomVariable> x =CreateObject<ExponentialRandomVariable>();
  x->SetAttribute("Mean",DoubleValue(packet_mean));
  uint32_t x_val=x->GetInteger();

  Ptr<UniformRandomVariable> y =CreateObject<UniformRandomVariable>();
  y->SetAttribute("Min",DoubleValue(0.0));
  y->SetAttribute("Max",DoubleValue(4.0));
  
  CommandLine cmd;
  cmd.AddValue ("nCsma10", "Number of \"extra\" CSMA nodes/devices in LAN 11", nWifi10);
...
uint32_t NGroupStas = NRawSta/NGroup;
  ...
  NetDeviceContainer wifiDevices10 = wifi.Install (wifiPhy, mac, wifiNodes10.Get (0));
...
for(int i = 1 ; i<=42;i++)
  {wifiDevices10.Add (wifi.Install (wifiPhy, mac, NodeContainer (wifiNodes10.Get (i))));}
...
/*
  ...
  csmaNodes10.Add(wifiNodes10.Get(3));
...
/*
  ...
  */
  ...
  //Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
   ...
   /*background traffic*/
  UdpServerHelper* udpServer = new UdpServerHelper[20];
  ApplicationContainer* serverApps = new ApplicationContainer[20];
  UdpClientHelper* udpClient = new UdpClientHelper[20];
  ApplicationContainer* clientApps = new ApplicationContainer[20];
  ...
  for (int i = 0; i <  20; i++)
    {
      ...
      clientApps[i] = udpClient[i].Install (wifiNodes10.Get (i + 25));
  ...
  if (tracing == true)
    {
      ...
      csma11.EnablePcap ("wifiah-40nodes-csma-http", csmaDevices11.Get (1), true);
      wifiPhy.EnablePcap ("wifiah-40nodes-wifi-http", wifiDevices10.Get (0), true);
    }
